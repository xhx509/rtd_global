...
queued_wifi = os.listdir(setup_rtd.parameters['path'] + 'queued/')
...
if len(queued_wifi) > 0:
    ...
    for elem in queued_wifi:
        print(elem)
        df_queue = pd.read_csv(setup_rtd.parameters['path'] + 'queued/' + elem, compression='gzip', error_bad_lines=False)
        #print(df_queue)
        ...
        if diff <= 1200:
            ...
            if conn_type == 1 or conn_type == 2:  # wifi or gsm
                Transfer('/home/ec2-user/rtd/vessels/{vessel}/'.format(vessel=setup_rtd.parameters['vessel_name'])).upload(
                    'queued/' + elem, 'queued/' + elem)
                os.remove(setup_rtd.parameters['path'] + 'queued/' + elem)
        else:
